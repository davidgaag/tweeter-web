...
const StoryHandler = async (event) => {
    ...
    let response = new tweeter_shared_1.LoadMoreItemsResponse(true, ...await new StatusService_1.StatusService(new DynamoDaoFactory_1.DynamoDaoFactory()).loadMoreStoryItems(request.authToken, request.user, request.pageSize, request.lastItem), "Load more story items successful");
    return response;
};
